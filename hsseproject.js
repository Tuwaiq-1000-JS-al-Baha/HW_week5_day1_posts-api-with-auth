  ...
  axios
    ...
    .then((Response) => {
      ...
      const postsSorted = posts.sort((postA, postB) =>
        postB.dateCreated.localeCompare(postA.dateCreated)
      );
        ...
        let postElement = `
                <div class="post-item">
                   <div class="post-info-${post._id}">
                       ...
                       <img src="${post.image}" widht=200px">
                       <p>Body: ${post.body}</p>
...
function addPost(event) {
  ...
  const owner = addForm.elements.image.value;
  const image = addForm.elements.owner.value;
  ...
  axios
    .put("https://vast-chamber-06347.herokuapp.com/api/posts/", postBody)
    .then((response) => {
...
function addComment(event, id) {
  ...
  axios.post(
    `https://vast-chamber-06347.herokuapp.com/api/posts/${id}/comment`,
    commentBody
  )
    .then((response) => {
